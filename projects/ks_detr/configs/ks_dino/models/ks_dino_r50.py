...
model = L(KSDINO)(
    ...
    transformer=L(KSDINOTransformer)(
...
if model.aux_loss:
    ...
    # =-----------------
    num_multi_head = 4
    for i in range(num_multi_head): # update to quad head in the last encoder layer
        #     output["enc_outputs" + f"_{i}"]
        aux_weight_dict.update({k + "_enc" + f"_{i}": v for k, v in base_weight_dict.items()})
    # =-----------------

    # num_layers = model.transformer.decoder.num_layers
    num_layers = get_num_of_layer(model.transformer.decoder.decoder_layer_config)
    multiplier = 3
    # for i in range(model.transformer.decoder.num_layers - 1):
    for i in range(num_layers * multiplier - 1):
