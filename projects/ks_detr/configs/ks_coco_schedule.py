...
def default_ks_coco_scheduler(epochs=60, decay_epochs=None, warmup_epochs=0):
    """
    Returns the config for a default multi-step LR scheduler such as "50epochs",
    commonly referred to in papers, where every 1x has the total length of 1440k
    training images (~12 COCO epochs). LR is decayed once at the end of training.
    ...
    if decay_epochs is None:
        decay_epochs = [50, 57]
    assert len(decay_epochs) == 2

    total_steps_16bs = epochs * 7500
    decay_steps1 = decay_epochs[0] * 7500
    decay_steps2 = decay_epochs[1] * 7500

    warmup_steps = warmup_epochs * 7500
    scheduler = L(MultiStepParamScheduler)(
        values=[1.0, 0.1, 0.01],
        milestones=[decay_steps1, decay_steps2, total_steps_16bs],
    )
    return L(WarmupParamScheduler)(
        scheduler=scheduler,
        warmup_length=warmup_steps / total_steps_16bs,
...
ks_lr_multiplier_60ep = default_ks_coco_scheduler(60, decay_epochs=[50, 57], warmup_epochs=0)
ks_lr_multiplier_65ep = default_ks_coco_scheduler(65, decay_epochs=[55, 62], warmup_epochs=0)
ks_lr_multiplier_70ep = default_ks_coco_scheduler(70, decay_epochs=[60, 67], warmup_epochs=0)
