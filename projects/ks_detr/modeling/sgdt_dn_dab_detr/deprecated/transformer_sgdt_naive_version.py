...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, d_model=256):
        ...
        self.sgdt = SGDT_module(embed_dim=d_model)  # ------------------- TTI Modification
    ...
    def forward(self, src,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # ------------------- TTI Modification
            # if layer_id < len(self.layers) - 1:  # torch.Size([800, 2, 256])
            #     # token adaption here, input: a set of tokens, output: a set of tokens.
            #     output = self.sgdt(output)

            if layer_id == len(self.layers) - 2:  # torch.Size([800, 2, 256])
                # token adaption here, input: a set of tokens, output: a set of tokens.
                output = self.sgdt(output)
            # -------------------
