...
class KSDNDetrTransformerEncoder(KSTransformerLayerSequence):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            encoder_intermediate_output_list.append(intermediate_output)
...
#
#
# class KSDNDetrTransformerEncoderWithGt(KSDNDetrTransformerEncoder):
#     def __init__(
#             self,
#             embed_dim: int = 256,
#             num_heads: int = 8,
...
#
#     ):
#         super(KSDNDetrTransformerEncoderWithGt, self).__init__(
#             encoder_layer_config=encoder_layer_config,
#             embed_dim=embed_dim,
#             num_heads=num_heads,
#             attn_dropout=attn_dropout,
...
def forward_additional_attn_out(transformer, target, pos_embed, anchor_box_embed, attn_mask,
                                intermediate_output_dict,
                                hidden_state, references,
                                **kwargs):
    assert 'encoder_intermediate_output_list' in intermediate_output_dict
        ...
        for encoder_out in encoder_intermediate_output_list:
            if 'feat_t' in encoder_out:
                ...
                for teacher_memory in teacher_memories:
                    hs_t, references_t, *decoder_intermediate_output_list_t = transformer.decoder(
                        query=target,
                        key=teacher_memory,
...
class KSDNDetrMultiAttnTransformer(KSDNDetrTransformer):
    """
    Dual or Triple Attention Transformer KSDNDetrMultiAttnTransformer
    """

    def forward_additional_attn_out(
            self, target, pos_embed, anchor_box_embed, attn_mask,
            intermediate_output_dict,
            hidden_state, references,
            **kwargs):
        return forward_additional_attn_out(
            transformer=self,
            target=target, pos_embed=pos_embed,
            anchor_box_embed=anchor_box_embed, attn_mask=attn_mask,
            # encoder_intermediate_output_list=encoder_intermediate_output_list,
