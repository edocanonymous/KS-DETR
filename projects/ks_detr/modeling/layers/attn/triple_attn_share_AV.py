...
def linear_(q_, w_q_, b_q_, freeze_=False):
    if freeze_:
...
class MultiheadTripleAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        if not self._qkv_same_embed_dim:
            ...
            self.in_proj_weight = Parameter(torch.empty((6 * embed_dim, embed_dim), **factory_kwargs))
        ...
        if bias: # go here
            ...
            self.in_proj_bias = Parameter(torch.empty(6 * embed_dim, **factory_kwargs))
        else:
    ...
    Outputs:
        ...
        if self.batch_first and is_batched: # safe, no into loop
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights, attn_output_weights_logits = multi_head_attention_forward(
                query, key, value,
                query_teacher, key_teacher, value_teacher,
                ...
                q_teacher_proj_weight=self.q_teacher_proj_weight,
                k_teacher_proj_weight=self.k_teacher_proj_weight,
                v_teacher_proj_weight=self.v_teacher_proj_weight,
...
def _in_projection_packed(
    ...
    if k is v:  # will not go to this branch
        ...
        w_q, w_k, w_v, w_q_teacher, w_k_teacher, w_v_teacher = w.chunk(6)
        if b is None:
            b_q = b_k = b_v = b_q_teacher = b_k_teacher = b_v_teacher = None
        else:
            b_q, b_k, b_v, b_q_teacher, b_k_teacher, b_v_teacher = b.chunk(6)

        Q = linear_(q, w_q, b_q)
        K = linear_(k, w_k, b_k)
        V = linear_(v, w_v, b_v)
        Q_teacher = linear_(q_teacher, w_q_teacher, b_q_teacher)
        K_teacher = linear_(k_teacher, w_k_teacher, b_k_teacher)
        V_teacher = linear_(v_teacher, w_v_teacher, b_v_teacher)
...
) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:  # -> Tuple[Tensor, Tensor, Tensor]:
    r"""
...
def multi_head_attention_forward(
        ...
        # out_proj_weight_teacher: Tensor,
        # out_proj_bias_teacher: Optional[Tensor],
        training: bool = True,
        key_padding_mask: Optional[Tensor] = None,
        ...
        v_teacher_proj_weight: Optional[Tensor] = None,
    ...
    #
    if not use_separate_proj_weight:  # # go here
        assert in_proj_weight is not None, "use_separate_proj_weight is False but in_proj_weight is None"
        # q, k, v = _in_projection_packed(query, key, value, in_proj_weight, in_proj_bias)
        q, k, v, q_teacher, k_teacher, v_teacher = _in_projection_packed(query, key, value,
                                                                         query_teacher, key_teacher, value_teacher,
        ...
        if in_proj_bias is None:
            ...
            b_q, b_k, b_v, b_q_teacher, b_k_teacher, b_v_teacher = in_proj_bias.chunk(6)
        # q, k, v = _in_projection(query, key, value, q_proj_weight, k_proj_weight, v_proj_weight, b_q, b_k, b_v)
        q, k, v, q_teacher, k_teacher, v_teacher = _in_projection(
            q=query, k=key, v=value,
            q_teacher=query_teacher, k_teacher=key_teacher,
            v_teacher=value_teacher,
            w_q=q_proj_weight, w_k=k_proj_weight,
            w_v=v_proj_weight,
            w_q_teacher=q_teacher_proj_weight,
            w_k_teacher=k_teacher_proj_weight,
            w_v_teacher=v_teacher_proj_weight,
            b_q=b_q, b_k=b_k, b_v=b_v,
            b_q_teacher=b_q_teacher, b_k_teacher=b_k_teacher,
            b_v_teacher=b_v_teacher
        )
    ...
    if bias_k is not None and bias_v is not None:
        ...
        k_teacher = torch.cat([k_teacher, bias_k.repeat(1, bsz, 1)])
        v_teacher = torch.cat([v_teacher, bias_v.repeat(1, bsz, 1)])
    ...
    if add_zero_attn:  # not go into the loop
        zero_attn_shape = (bsz * num_heads, 1, head_dim)
        k = torch.cat([k, torch.zeros(zero_attn_shape, dtype=k.dtype, device=k.device)], dim=1)
        v = torch.cat([v, torch.zeros(zero_attn_shape, dtype=v.dtype, device=v.device)], dim=1)

        k_teacher = torch.cat([k_teacher, torch.zeros(zero_attn_shape, dtype=k_teacher.dtype, device=k_teacher.device)],
                              dim=1)
        v_teacher = torch.cat([v_teacher, torch.zeros(zero_attn_shape, dtype=v_teacher.dtype, device=v_teacher.device)],
                              dim=1)
    ...
    # =================================
    attn_output_teacher1 = attn_output_teacher1.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
    # attn_output_teacher = linear(attn_output_teacher, out_proj_weight_teacher, out_proj_bias_teacher)
    attn_output_teacher1 = linear(attn_output_teacher1, out_proj_weight, out_proj_bias)
    attn_output_teacher1 = attn_output_teacher1.view(tgt_len, bsz, attn_output_teacher1.size(1))

    attn_output_teacher2 = attn_output_teacher2.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
    attn_output_teacher2 = linear(attn_output_teacher2, out_proj_weight, out_proj_bias)
    attn_output_teacher2 = attn_output_teacher2.view(tgt_len, bsz, attn_output_teacher2.size(1))
    ...
    if need_weights:
        ...
        if not is_batched: # safe, not into the loop
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
            attn_output_teacher1 = attn_output_teacher1.squeeze(1)
            attn_output_teacher2 = attn_output_teacher2.squeeze(1)

            attn_output_weights = attn_output_weights.squeeze(0)
            attn_output_weights_teacher = attn_output_weights_teacher.squeeze(0)
        return (attn_output, attn_output_teacher1, attn_output_teacher2), \
               (attn_output_weights, attn_output_weights_teacher, attn_output_weights), \
               (attn_output_weights_logits, attn_output_weights_logits_teacher, attn_output_weights_logits)
    else:
        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
            attn_output_teacher1 = attn_output_teacher1.squeeze(1)
            attn_output_teacher2 = attn_output_teacher2.squeeze(1)
        return (attn_output, attn_output_teacher1, attn_output_teacher2), None, None
