...
class MultiheadDualAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.q_teacher_proj_weight, self.k_teacher_proj_weight, self.v_teacher_proj_weight = None, None, None
        self.q_teacher_in_proj_bias, self.k_teacher_in_proj_bias, self.v_teacher_in_proj_bias = None, None, None
        ...
        if add_bias_kv and not (self.share_k or self.share_attn_map) and not self.share_v:
            self.k_teacher_bias = Parameter(torch.empty((1, 1, embed_dim), **factory_kwargs))
            self.v_teacher_bias = Parameter(torch.empty((1, 1, embed_dim), **factory_kwargs))
        else:
            self.k_teacher_bias = self.v_teacher_bias = None
    ...
    def _reset_parameters(self):
        ...
        if self.k_teacher_bias is not None:
            xavier_normal_(self.k_teacher_bias)
        if self.v_teacher_bias is not None:
            xavier_normal_(self.v_teacher_bias)
