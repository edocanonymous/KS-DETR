...
class MultiheadDynamicAttention(Module):
    r"""Allows the model to change attention type during training, e.g.,
    TripleAttention to PlainAttention.
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.q_teacher_in_proj_weight = Parameter(torch.empty((embed_dim, embed_dim), **factory_kwargs))
        self.k_teacher_in_proj_weight = Parameter(torch.empty((embed_dim, self.kdim), **factory_kwargs))
        self.v_teacher_in_proj_weight = Parameter(torch.empty((embed_dim, self.vdim), **factory_kwargs))
        if bias:
        ...
        if add_bias_kv:
            ...
            self.bias_k_teacher = self.bias_v_teacher = None
    ...
    def _reset_parameters(self):
        ...
        _init_proj_weight(self.q_teacher_in_proj_weight)
        _init_proj_weight(self.k_teacher_in_proj_weight)
        _init_proj_weight(self.v_teacher_in_proj_weight)
    ...
    Outputs:
        ...
        assert dynamic_attn_config in DYNAMIC_ATTN_LIST
        ...
        in_proj_weight_teacher = [self.q_teacher_in_proj_weight, self.k_teacher_in_proj_weight,
                                  self.v_teacher_in_proj_weight]
        ...
        attn_output, attn_output_weights, attn_output_weights_logits = multi_head_attention_forward(
            dynamic_attn_config=dynamic_attn_config,
            query=query, key=key, value=value,
            query_teacher=query_teacher, key_teacher=key_teacher, value_teacher=value_teacher,
    ...
    def reshape_teacher_qkv(self, q_teacher, k_teacher, v_teacher, tgt_len, bsz, num_heads, head_dim, E,
                            static_k, static_v, add_zero_attn):
        q_teacher_scaled = None
        ...
        if k_teacher is not None:
            ...
            if add_zero_attn:
                ...
                k_teacher = torch.cat([k_teacher, torch.zeros(
                    zero_attn_shape,
                    dtype=k_teacher.dtype, device=k_teacher.device)],
                                      dim=1)
        ...
        return q_teacher_scaled, k_teacher, v_teacher


class MultiheadAttentionShareASeparateWeight(MultiheadDynamicAttention):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.q_teacher_proj_weight = self.q_teacher_in_proj_bias = None
        self.k_teacher_proj_weight = self.k_teacher_in_proj_bias = self.bias_k_teacher = None

        self.default_dynamic_attn_config = 'DualAttentionShareA'


class MultiheadAttentionShareVSeparateWeight(MultiheadDynamicAttention):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.v_teacher_proj_weight = self.v_teacher_bias = self.bias_v_teacher = None
        self.default_dynamic_attn_config = 'DualAttentionShareV'


class MultiheadTripleAttentionSeparateWeight(MultiheadDynamicAttention):
...
def _add_not_used(out_, qkv_list_):
    for q_ in qkv_list_:
        if q_ is not None:
            out_ = out_ + 0 * q_.sum()
    return out_
...
def multi_head_attention_forward(
        ...
        # q_teacher_proj_weight: Optional[Tensor] = None,
        # k_teacher_proj_weight: Optional[Tensor] = None,
    ...
    def out_proj_(attn_output_):
        attn_output_ = attn_output_.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
        attn_output_ = linear(attn_output_, out_proj_weight, out_proj_bias)
        attn_output_ = attn_output_.view(tgt_len, bsz, attn_output_.size(1))
        return attn_output_

    attn_output = out_proj_(attn_output)

    # --------------------------
    # other attention branches
    # --------------------------
    q_teacher_scaled = None
    if q_teacher is not None:
        q_teacher = q_teacher.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
    ...
    attn_output_weights_t, attn_output_weights_logits_t, attn_output_t = [], [], []

    # We can dynamically change the attention type, e.g., from TripleAttention to PlainAttention,
    # or DualAttentionShareV to PlainAttention. To make backpropagation work, we use the unused
    # q_teacher, k_teacher, v_teacher by, e.g.,  0 * q_teacher.sum().
    if dynamic_attn_config == 'PlainAttention':
        # q_teacher, k_teacher, v_teacher not used.
        attn_output = _add_not_used(attn_output, [q_teacher, k_teacher, v_teacher])
        ...
        if dynamic_attn_config == 'DualAttentionShareV':
            ...
            attn_output_teacher = out_proj_(attn_output_teacher)

            attn_output_teacher = _add_not_used(attn_output_teacher, [v_teacher])

            attn_output_weights_logits_t += [attn_output_weights_logits_teacher]
            attn_output_weights_t += [attn_output_weights_teacher]
            attn_output_t += [attn_output_teacher]

        if dynamic_attn_config == 'TripleAttention':
            attn_output_teacher1 = torch.bmm(attn_output_weights_teacher, v)  # teacher 1 (share V), A^t V
            attn_output_teacher2 = torch.bmm(attn_output_weights, v_teacher)  # teacher 2 (share A), A V^t

            attn_output_teacher1 = out_proj_(attn_output_teacher1)
            attn_output_teacher2 = out_proj_(attn_output_teacher2)

            attn_output_weights_logits_t += [attn_output_weights_logits_teacher, attn_output_weights_logits]
            attn_output_weights_t += [attn_output_weights_teacher, attn_output_weights]
            attn_output_t += [attn_output_teacher1, attn_output_teacher2]

    elif dynamic_attn_config == 'DualAttentionShareA':
        attn_output_teacher = torch.bmm(attn_output_weights, v_teacher)
        attn_output_teacher = out_proj_(attn_output_teacher)

        # q_teacher, k_teacher not used in dynamic attention
        attn_output_teacher = _add_not_used(attn_output_teacher, [q_teacher, k_teacher, ])

        attn_output_weights_logits_t += [attn_output_weights_logits]
        attn_output_weights_t += [attn_output_weights]
        attn_output_t += [attn_output_teacher]
    else:
        raise NotImplementedError

    assert len(attn_output_weights_logits_t) == len(attn_output_weights_t) and \
           len(attn_output_weights_t) == len(attn_output_t)

    if need_weights:
        # optionally average attention weights over heads
        attn_output_weights = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
        attn_output_weights_t = [x.view(bsz, num_heads, tgt_len, src_len) for x in attn_output_weights_t]

        attn_output_weights_logits = attn_output_weights_logits.view(bsz, num_heads, tgt_len, src_len)
        attn_output_weights_logits_t = [x.view(bsz, num_heads, tgt_len, src_len) for x in attn_output_weights_logits_t]

        if average_attn_weights:
            attn_output_weights = attn_output_weights.sum(dim=1) / num_heads
            attn_output_weights_t = [x.sum(dim=1) / num_heads for x in attn_output_weights_t]
        ...
        if not is_batched:
            ...
            attn_output_t = [x.squeeze(1) for x in attn_output_t]
            attn_output_weights_t = [x.squeeze(0) for x in attn_output_weights_t]

        if len(attn_output_weights_t) == 0:
            return attn_output, attn_output_weights, attn_output_weights_logits
        else:
            return (attn_output,) + tuple(attn_output_t), \
                   (attn_output_weights,) + tuple(attn_output_weights_t), \
                   (attn_output_weights_logits,) + tuple(attn_output_weights_logits_t)
    else:
        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
        ...
        if len(attn_output_weights_t) == 0:
            ...
            return (attn_output,) + tuple(attn_output_t), None, None
