...
class KSDNDETR(DNDETR):
    ...
    def __init__(
        ...
        self.ksgt = ksgt
        # TODO: future work to write the initialization function for ksgt
        # self.ksgt.init_weights()

    # def init_ksgt(self, img_masks, features, targets, padded_img_size):
    #     if self.ksgt:  # prepare fg bg mask if necessary
    #         # (B, H, W), [2, 21, 29]; to change to B, N use .flatten(1)
    #         self.ksgt.src_key_padding_mask = img_masks
    #
    #         bs, c, h, w = features.shape  # torch.Size([2, 256, 25, 32])
    #         self.ksgt.feat_map_size = (h, w)
    #         # self.ksgt.targets = targets
    #
    #         self.ksgt.set_ksgt_targets(targets, feat_map_size=(h, w), padded_img_size=padded_img_size)
    #
    #         # if self.training:
    ...
    def forward(self, batched_inputs):
        ...
        gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        targets = self.prepare_targets(gt_instances)
        # self.init_ksgt(img_masks=img_masks, features=features, targets=targets,
        #                padded_img_size=(H, W))
        ...
        if self.training:
            ...
            # if self.ksgt and self.ksgt.eval_decoder_layer != -1:  # specify which decoder layer to evaluate
            #     eval_decoder_layer = self.ksgt.eval_decoder_layer
            #     if self.aux_loss and eval_decoder_layer < len(output["aux_outputs"]):
