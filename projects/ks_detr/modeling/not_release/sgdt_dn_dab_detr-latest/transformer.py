...
from models.sgdt.attn_ import MultiheadAttention
from models.sgdt.dual_attn import MultiheadDualAttention
...
def get_self_attn_q_k_v(src,
    ...
    # 'X', 'Q', 'K', 'V', 'QK', 'KV', 'MHA_Out', 'FFN_Out'  # in
    # q = k = v = None  # This is a big bug
...
class TransformerEncoderDualAttnLayerShareVOutProjFFN(TransformerEncoderLayer1):
    ...
    def forward_attn(self,
        ...
        # src2_s_t_tuple, _, attn_output_weight_logits_s_t_tuple = self.self_attn(
        #     q, k, value=src,  # never use value = v and v = src.
        #     query_teacher=torch.zeros_like(q),
        #     key_teacher=torch.zeros_like(k),  # k.clone().detach()
        #     value_teacher=src,
        #     attn_mask=src_mask,
        #     key_padding_mask=src_key_padding_mask,
        #     need_weights=True, average_attn_weights=False,
    ...
    def forward(self,
        ...
        # # return src_s * torch.zeros_like(src_s) + final_src, attn_map_logits, \
        # #        src_t * torch.zeros_like(src_t) + final_src, attn_output_weight_logits_teacher

class TransformerEncoderWithGTFeatureLayer(TransformerEncoderLayer1):
    ...
    def forward(self,
        ...
        src2, _, attn_output_weight_logits = self.self_attn(q, k, value=src, attn_mask=src_mask,
                                                            ...
                                                            need_weights=True, average_attn_weights=False)
        # ################################################
        if sgdt.args.token_masking and sgdt.args.token_masking_loc == 'MHA_out':
        ...
        if sgdt.args.token_masking and sgdt.args.token_masking_loc == 'MHA_feature':
        ...
        if sgdt.args.token_masking and sgdt.args.token_masking_loc == 'FFN_out':
            src2 = get_token_gt_masking(
                src=src2, token_masking=sgdt.args.token_masking,
                src_key_padding_mask=src_key_padding_mask, sgdt=sgdt)
        ...
        if sgdt.args.token_masking and sgdt.args.token_masking_loc == 'FFN_feature':
            src = get_token_gt_masking(
                src=src, token_masking=sgdt.args.token_masking,
                src_key_padding_mask=src_key_padding_mask, sgdt=sgdt)
...
class TransformerEncoderWithGTLayer(TransformerEncoderLayer1):
    # Q from adapted tokens, k, v from original token
    ...
    def forward(self,
        ...
        # sgdt_output = sgdt(x=src, mask=src_key_padding_mask)
        # k_adapted_pos = pos
        # q = k = with_pos_embed(sgdt_output['x'], pos=k_adapted_pos)
...
class TransformerEncoderLayerSTSGDTShareVOutProjFFN(TransformerEncoderLayerSTMarkFgBgShareVNoFreeze):
    ...
    def forward(self,
        ...
        return src, attn_map_logits, src_t, attn_output_weight_logits_teacher
        #
        # return src * torch.zeros_like(src) + final_src, attn_map_logits, \
        #        src_t * torch.zeros_like(src_t) + final_src, attn_output_weight_logits_teacher
...
class TransformerEncoder(nn.Module):
    ...
    def forward_subset_encoder_layers(self, src,
        ...
        for layer_id, layer in enumerate(encoder_layers):
            ...
            pos_scales = self.query_scale(output) # * torch.zeros_like(output) + torch.ones_like(output)
            ...
            if type(layer).__name__ == 'TransformerEncoderLayer':  # Regular TransformerEncoderLayer
                ...
                'TransformerEncoderWithGTLayer',
                'TransformerEncoderWithGTFeatureLayer',
                'TransformerEncoderMarkFgBgLayer', 'TransformerEncoderRandomMarkFgBgLayer']:
                output, attn_map_logits = layer(
                    output, src_mask=mask,
                    src_key_padding_mask=src_key_padding_mask,
                    pos=pos * pos_scales,  # scaled pos is passed in
...
class TransformerShareDoubleHead(nn.Module):
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed, tgt, attn_mask=None,
        ...
        # hs, references = self.decoder(tgt, teacher_memory, tgt_mask=attn_mask, memory_key_padding_mask=mask,
        #                               pos=pos_embed, refpoints_unsigmoid=refpoint_embed,
        #                               sgdt=sgdt,
        #                               )
        #
