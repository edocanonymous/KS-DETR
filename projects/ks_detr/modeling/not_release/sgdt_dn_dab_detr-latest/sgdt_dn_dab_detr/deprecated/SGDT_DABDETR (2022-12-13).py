...
class SetCriterion(SetCriterionOld):
    ...
    def cal_decoder_distill_loss(self, outputs, targets,
        ...
        teacher_pred = {k: v for k, v in targets.items() if k != 'aux_outputs'}
        ...
        for loss in self.losses:
            losses.update(self._get_decoder_distill_loss(loss, outputs, targets=teacher_pred,
                                                         ))
        ...
        #                 kwargs = {'log': False}
        #             l_dict = self._get_decoder_distill_loss(loss, aux_outputs, targets,
    ...
    def _get_decoder_distill_loss(self, loss, outputs, targets,
                                  # indices, num_boxes, **kwargs
                                  ):
        loss_map = {
    ...
    def decoder_distill_loss_labels(self, outputs, targets,
        ...
        target_logits = targets['pred_logits']

        # * valid_mask).sum() / (bsz * num_heads * src_len * weight)
        loss_kl = F.kl_div(F.log_softmax(src_logits, dim=-1, dtype=torch.float32),
        ...
        losses = {'sgdt_decoder_pred_cls': loss_kl}
        # if log:
    ...
    def decoder_distill_loss_boxes(self, outputs, targets,
        ...
        losses = {}
        batch_size, num_boxes = target_boxes.shape[:1]  # 'pred_boxes' torch.Size([2, 300, 4])
        n = batch_size *  num_boxes
        ...
        losses['sgdt_decoder_loc_cls'] = loss_bbox.sum() / n + loss_giou.sum() / n
