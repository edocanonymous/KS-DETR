...
class KSDINODetrMultiAttnTransformer(KSDINOTransformer):
    ...
    def _extract_enc_single_attn_head_out(
            self, memory, mask_flatten, spatial_shapes, query_embed, bs):
        """
        the following variables will be used in decoder from memoery
        target,  (query)
        memory,
        reference_points
        ...
        memory -> output_memory, output_proposals
                output_memory -> enc_outputs_class
                output_memory + output_proposals ->enc_outputs_coord_unact

                                enc_outputs_class -> topk_proposals
                                                    enc_outputs_coord_unact + topk_proposals -> topk_coords_unact
                                                    topk_coords_unact -> reference_points
                            output_memory,  topk_proposals -> target_unact
                                                    target_unact ->         target
        ...
        return target_unact, topk_coords_unact, init_reference_out, reference_points, target

    def append_teacher_encoder_output(
            self, mask_flatten, spatial_shapes, query_embed,
            encoder_intermediate_output_list, target_unact, topk_coords_unact, init_reference_out, bs):
        ...
        if encoder_intermediate_output_list:
            ...
            if 'feat_t' in encoder_out:
                ...
                # encoder_out['init_reference_out_t'] = [None] * len(teacher_memories)
                encoder_out['reference_points_t'] = [None] * len(teacher_memories)
                encoder_out['target_t'] = [None] * len(teacher_memories)

                for k, teacher_memory in enumerate(teacher_memories):
                    # ----------------------
                    # Important note: Never update init_reference_out
                    # target_unact, topk_coords_unact, init_reference_out, reference_points, target
                    target_unact_t, topk_coords_unact_t, init_reference_out_t, reference_points_t, target_t = \
                        self._extract_enc_single_attn_head_out(
                    ...
                    init_reference_out.append(init_reference_out_t)

                    # TODO: check if encoder_intermediate_output_list is updated as well.
                    # encoder_out['init_reference_out_t'][k] = init_reference_out_t
                    encoder_out['reference_points_t'][k] = reference_points_t
                    encoder_out['target_t'][k] = target_t

        return target_unact, topk_coords_unact, init_reference_out, encoder_intermediate_output_list
    ...
    def forward(
        ...
        target_unact, topk_coords_unact, init_reference_out, encoder_intermediate_output_list = self.append_teacher_encoder_output(
        ...
        if encoder_intermediate_output_list:
            ...
            for encoder_out in encoder_intermediate_output_list:
                if 'feat_t' in encoder_out:
                    ...
                    count += 1
                    for k, teacher_memory in enumerate(teacher_memories):
                        reference_points_t = encoder_out['reference_points_t'][k]
                        target_t = encoder_out['target_t'][k]

                        # ----------------------
                        # decoder
                        inter_states_t, inter_references_t, *decoder_intermediate_output_list_t = self.decoder(
                            query=target_t,  # bs, num_queries, embed_dims # -- target
                            key=teacher_memory,  # bs, num_tokens, embed_dims  # -- key is not used even passed.
                            value=teacher_memory,  # bs, num_tokens, embed_dims
                            query_pos=None,
                            key_padding_mask=mask_flatten,  # bs, num_tokens
                            reference_points=reference_points_t,  # num_queries, 4  # -- reference_points
                            spatial_shapes=spatial_shapes,  # nlvl, 2
                            level_start_index=level_start_index,  # nlvl
                            valid_ratios=valid_ratios,  # bs, nlvl, 2
                            attn_masks=attn_masks,
...
class KSDINODetrMultiAttnTransformerV2ShareOnlyTeacherMemory(KSDINOTransformer):
            ...
            for encoder_out in encoder_intermediate_output_list:
                if 'feat_t' in encoder_out:
                    ...
                    for k, teacher_memory in enumerate(teacher_memories):
                        ...
                        # decoder
                        inter_states_t, inter_references_t, *decoder_intermediate_output_list_t = self.decoder(
                            query=target,  # bs, num_queries, embed_dims # -- target
                            key=teacher_memory,  # bs, num_tokens, embed_dims  # -- key is not used even passed.
                            value=teacher_memory,  # bs, num_tokens, embed_dims
                            query_pos=None,
                            key_padding_mask=mask_flatten,  # bs, num_tokens
                            reference_points=reference_points,  # num_queries, 4  # -- reference_points
                            spatial_shapes=spatial_shapes,  # nlvl, 2
                            level_start_index=level_start_index,  # nlvl
                            valid_ratios=valid_ratios,  # bs, nlvl, 2
                            attn_masks=attn_masks,
...
class KSDINODetrMultiAttnTransformerV1ShareReferencePoint(KSDINODetrMultiAttnTransformer):
            ...
            for encoder_out in encoder_intermediate_output_list:
                if 'feat_t' in encoder_out:
                    ...
                    for k, teacher_memory in enumerate(teacher_memories):
                        ...
                        # ---------------------- update the reference points even they are not used outside
                        encoder_out['reference_points_t'][k] = reference_points
