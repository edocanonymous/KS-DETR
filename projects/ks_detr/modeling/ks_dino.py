...
class KSDINO(DINO):
    ...
    def extract_output_v0(self, inter_states, init_reference, inter_references, ksgt=None):
    ...
    def extract_output(self, inter_states, init_reference, inter_references, ksgt=None):
        ...
        #  need to handle multiple class_embed, bbox_embed in the output of the transformer.
        # init_reference is always a tensor, and the decoder output share the same init_reference
        if isinstance(inter_states, list) and isinstance(inter_references, list) and \
                isinstance(init_reference, list):
            outputs_class = []
            outputs_coord = []
            ...
            for k, (single_decoder_inter_state, single_decoder_inter_references, single_init_reference) \
                    in enumerate(zip(inter_states, inter_references, init_reference)):
                outputs_classes_single, outputs_coords_single = self._extract_predict(
                    inter_states=single_decoder_inter_state,
                    init_reference=single_init_reference,
                    inter_references=single_decoder_inter_references)
                # The first output is from the decoder of the normal attention.
                # The output of teacher attentions are from the second one.
                if k > 0 and teacher_attn_return_no_intermediate_out:
    ...
    def forward(self, batched_inputs):
        ...
        if isinstance(enc_reference, list) and isinstance(enc_state, list):
            interm_coord = enc_reference[0]
            interm_class = self.transformer.decoder.class_embed[-1](enc_state[0])
            output["enc_outputs"] = {"pred_logits": interm_class, "pred_boxes": interm_coord}

            # Handling the output of the teacher branches.
            for i, (enc_ref, enc_st) in enumerate(zip(enc_reference[1:], enc_state[1:])):
                interm_coord = enc_ref
                interm_class = self.transformer.decoder.class_embed[-1](enc_st)
                output["enc_outputs" + f"_{i}"] = {"pred_logits": interm_class, "pred_boxes": interm_coord}
        else:
            assert torch.is_tensor(enc_reference) and torch.is_tensor(enc_state)
