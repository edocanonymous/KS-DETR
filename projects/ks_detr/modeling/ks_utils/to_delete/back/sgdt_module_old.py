...
class TokenScoringVersionParser:
    def __init__(self, token_scoring_version):
        self.token_scoring_version = token_scoring_version

    def reclaim_padded_region_(self):
        if self.token_scoring_version.find('reclaim_padded_region') > -1:
            return True
        else:
            return False

    def ignore_bg_label_(self):
        if self.token_scoring_version.find('ignore_bg_label') > -1:
            return True
        else:
            return False
    ...
    def pred_score_(self):
        ...
        if  token_scoring_version in ['v0_with_gt',
        ...
        """
        if self.token_scoring_version.find('gt') > -1:
            return False
        else:
            return True
...
class SGDT_module(nn.Module):
    ...
    def __init__(self, embed_dim, max_split_token_num=10000, token_scoring_version=None,
                 ):  # no restriction with 10000
        super().__init__()
        self.token_scoring_version = token_scoring_version
        self.token_scoring_version_parser = TokenScoringVersionParser(token_scoring_version)
        if token_scoring_version is not None and self.token_scoring_version_parser.pred_score_():
    ...
    def forward(self, x, mask=None,
        ...
        if self.token_scoring_version == 'v0_with_gt':
        ...
        elif self.token_scoring_version == 'v0_with_gt_and_reclaim_padded_region':
            return self.discard_split_with_gt_token_score(
                x, mask, reclaim_padded_region=True,
                sgdt_targets=sgdt_targets,
                ignore_bg_label=False
            )
        elif self.token_scoring_version == 'v0_with_gt_only_reclaim_padded_region':
            return self.discard_split_with_gt_token_score(
                x, mask, reclaim_padded_region=True,
                sgdt_targets=sgdt_targets,
                ignore_bg_label=True
            )
        elif self.token_scoring_version == 'v1_selection_differentiable':
            return self.discard_split(x, mask)
        else:
            raise NotImplementedError
