...
from models.sgdt.sgdn_components import MIN_FG_SIGNIFICANCE
...
class SGDT_module(nn.Module):
    ...
    def __init__(self, embed_dim, max_split_token_num=10000,
        ...
        if token_scoring_discard_split_criterion.find('dynamic_vit_pred_fg_only_remove') > -1:
            ...
            if token_scoring_discard_split_criterion is not None and \
                    self.token_scoring_discard_split_criterion_parser.pred_score_():
                if self.token_scoring_discard_split_criterion_parser.token_scoring_with_fg_pred_():
            ...
            if self.token_scoring_discard_split_criterion_parser.token_split_():
    ...
    def _pred_processing(self,
        ...
        if only_remove_bg_token_no_split:
            ...
            tokens_to_discard = tokens_to_discard_original

        else:
            # token sampling for background tokens
            if reclaim_padded_region:  # only modify tokens_to_discard_original
                if not ignore_bg_label:  # predicted bg for valid tokens + padded region
                    ...
                    fg_score[invalid_tokens] = float("-inf")  # -1e6  # set the score to be very lower
        ...
        output_dict = {
            ...
            'valid_tokens': valid_tokens_float,
    ...
    def discard_split_pred_fg_small_scale(
            self, x, mask=None,  #
            reclaim_padded_region=False,
            ignore_bg_label=False,
            only_remove_bg_token_no_split=False,
        ...
        # debug_remove_original=False
        if debug:
            # scale_gt =
            # fg_gt = torch.where(sgdt_targets['scale_gt'] > 0, 1, 0)  # torch.int64
            fg_gt = sgdt_targets['fg_gt'].bool()  # torch.int64
            # filter out false
    ...
    def forward(self, x, mask=None,
        ...
        # elif self.token_scoring_discard_split_criterion == 'pred_fg_only_remove_0.05':
        #     return self.discard_split_pred_fg_small_scale(
        #         x, mask,
        #         reclaim_padded_region=False,
        #         ignore_bg_label=False,
        #         only_remove_bg_token_no_split=True,
        #         max_bg_significance_thd=0.05,
        #     )
        elif self.token_scoring_discard_split_criterion == 'pred_fg_only_remove_0.1_w_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
                only_remove_bg_token_no_split=True,
                max_bg_significance_thd=0.1,
                with_global_feat=True,
                sgdt_targets=sgdt_targets,
            )
        elif self.token_scoring_discard_split_criterion == 'pred_fg_only_remove_0.1_wo_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
                only_remove_bg_token_no_split=True,
                max_bg_significance_thd=0.1,
                with_global_feat=False,
                sgdt_targets=sgdt_targets,
            )
        elif self.token_scoring_discard_split_criterion == 'pred_fg_only_remove_0.3_wo_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
        ...
        elif self.token_scoring_discard_split_criterion == 'pred_fg_only_remove_0.5_wo_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
                only_remove_bg_token_no_split=True,
                max_bg_significance_thd=0.5,
                with_global_feat=False,
                sgdt_targets=sgdt_targets,
            )
        elif self.token_scoring_discard_split_criterion == 'dynamic_vit_pred_fg_only_remove_0.1_w_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
        ...
        elif self.token_scoring_discard_split_criterion == 'dynamic_vit_pred_fg_only_remove_0.1_wo_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
        ...
        elif self.token_scoring_discard_split_criterion == 'dynamic_vit_pred_fg_only_remove_0.3_wo_gf':
            return self.discard_split_pred_fg_small_scale(
                x, mask,
                reclaim_padded_region=False,
                ignore_bg_label=False,
            ...
            with_global_feat = torch.tensor([v.find('w_gf') for v in discard_split_configs]).any()
            inverse_bg_thd = torch.tensor([v.find('inverse') for v in discard_split_configs]).any()

            max_bg_significance_thd = None
            for v in discard_split_configs:
                if v.find('min_thd') > -1:
                    max_bg_significance_thd = float(v.split('min_thd')[-1])
                    break
