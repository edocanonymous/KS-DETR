...
class SGDT_module(nn.Module):
    ...
    def discard_split_significance_all_fg_w_priority(
        ...
        fg_mask = (significance_score >= 0.5).float() * valid_tokens_float  # 0 bg, 1 fg. float
        bg_mask = 1 - fg_mask
    ...
    def _extract_adapted_token(self, x, tokens_to_discard_original,
        ...
        Args:
            ...
            tokens_small_obj_original: float
            significance_score:
            small_scale_score:
    ...
    def _reassemble_tokens(self, x, tokens_to_discard_original, tokens_small_obj_original,
        ...
        keep_mask = 1 - (
                tokens_to_discard_original * tokens_to_discard + tokens_small_obj_original * tokens_small_obj)
    ...
    def discard_split(self, x, mask=None, scale_fg_independent=True):
        """
        scale guided supervision differentiable, but sumbel_softmax also differentiable
        we need to decouple these two processes to check their effects.
        Using predicted labels as selection results is also a good choice. I need to test this as well.
        ...
        # require_grad = True for tokens_to_discard_original and tokens_small_obj_original
        ...
        if scale_fg_independent:
            ...
            tokens_small_obj_original_raw = scale_mask[:, :, 1] * valid_tokens
        else:
            tokens_small_obj_original_raw = fg_mask[:, :, 1] * scale_mask[:, :, 1] * valid_tokens

        # The tokens both to split or remove.
        ambiguous_tokens = tokens_to_discard_original_raw * tokens_small_obj_original_raw

        # Remove the ambiguous_tokens from the remove set and split set.
        tokens_to_discard_original = tokens_small_obj_original_raw - ambiguous_tokens
        tokens_small_obj_original = tokens_small_obj_original_raw - ambiguous_tokens
...
class SGDT_module_deprecated(SGDT_module):
    ...
    def discard_split_v1(self, x, mask=None, scale_fg_independent=True):
        """
        scale guided supervision differentiable, but sumbel_softmax also differentiable
        we need to decouple these two processes to check their effects.
        Using predicted labels as selection results is also a good choice. I need to test this as well.
        ...
        if scale_fg_independent:
            # keep scale_mask and fg_mask independent to simplify the training task.
            tokens_small_obj_original = scale_mask[:, :, 1] * valid_tokens
        else:
            tokens_small_obj_original = fg_mask[:, :, 1] * scale_mask[:, :, 1] * valid_tokens
        # tokens_large_obj = (fg_mask[:, :, 1] * scale_mask[:, :, 2]).bool()  # foreground + large objects
        # Generate new tokens from the old ones, filling the locations of tokens to discard with split tokens
