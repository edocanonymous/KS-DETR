...
from models.sgdt.scoring_gt import TokenScoringGTGenerator, ProposalScoringParser
from models.sgdt.scoring_loss import TokenScoringLoss
from models.sgdt.sgdt_components import SGDTConfigParse, extract_thd
...
class SGDT_module(nn.Module):
    ...
    def __init__(self, embed_dim, max_split_token_num=10000,  # 10000  300
                 max_split_token_num_inference=300,
        ...
        if self.token_scoring_config_parser.token_split_():
            if self.token_scoring_config_parser.str_exist('split_1c'):
...
,
    ...
    def discard_split_with_gt_token_score(self, x, mask=None,
        ...
        if use_proposal:
            # if self.token_scoring_config_parser.extract_thd(sub_str='debug_gt_split_ratio'):
...
class GTRatioUpdater:
    def __int__(self,
        ...
        assert gt_decay_criterion is not None
    ...
    def update_gt_ratio(self, epoch, cur_iter):
        if epoch < self.decay_start_epoch:
            return 1.0
        elif epoch >= self.decay_end_epoch:
            self.gt_ratio = 0
        else:
            cur_step = (epoch - self.decay_start_epoch) * self.data_size + cur_iter
...
class SGDT:
    def __init__(self, token_scoring_loss_criterion=None,
        ...
        if proposal_scoring is not None:
            ...
            self.proposal_processor = ProposalProcess(**proposal_filtering_param)
        else:
            self.proposal_processor = ProposalProcess()
    ...
    def update_proposal_gt(self, selected_proposals):
        ...
        self.targets, self.sgdt_target_raw = self.token_scoring_gt_generator.update_proposal_gt_raw(
            targets=self.targets, selected_proposals=selected_proposals, sgdt_target_raw=self.sgdt_target_raw)
        self.sgdt_targets = self.token_scoring_gt_generator.resize_sig_value_gt(
    ...
    def init_gt_ratio_updater(self, data_size, total_epoch, gt_decay_criterion):
        self.gt_ratio_updater = GTRatioUpdater(
            data_size=data_size,
            total_epoch=total_epoch,
            gt_decay_criterion=gt_decay_criterion
        )

    def update_gt_ratio(self, epoch, cur_iter):
        if self.gt_ratio_updater is not None:
            self.gt_ratio_updater.update_gt_ratio(epoch, cur_iter)
