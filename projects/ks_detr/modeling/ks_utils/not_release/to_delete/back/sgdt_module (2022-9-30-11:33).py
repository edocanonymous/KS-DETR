...
,
    ...
    def discard_split_topk_token_selection_differentiable(
        ...
        **kwargs examples:
        ...
        # not be selected for split.
        # N, B, C = x.shape
        # x_small = torch.zeros_like(x)
        # tokens_small_obj = torch.zeros_like(tokens_to_discard) * valid_tokens_float
        # split_token_num = torch.sum((1 - valid_tokens_float), dim=0).int()  # (N, B)
        # for k in range(B):
        #     # different image has different number of tokens to split or merge, thus
        #     # batch processing is not possible.
        #     top_k = split_token_num[k].item()  # must be int not tensor
        #     scores = significance_score[:, k].unsqueeze(0)  # N, ->  1, N (to fit the b, N format)
        #
        #     indicator = PerturbedTopKFunction.apply(
        #         scores,
        #         top_k, self.num_samples, 0.05)  # b k d : 1, top_k, N, e.g., torch.Size([1, 32, 736])
        ...
        #     x_k = patches.squeeze(0)  # top_k, c
        #     tokens_small_obj_new = self.token_split_conv(x_k)
        #
        #     indices = torch.argmax(indicator, dim=-1).squeeze(0)  # not differentiable, but the indices does not
        ...
        #     x_small[indices, k, :] += tokens_small_obj_new[0]  # '+' to make it differentiable
        ...
        # x_new1 = x.clone() * keep_mask.unsqueeze(-1).repeat(1, 1, x.shape[-1]) + x_small  # t

        if kwargs.get('sigma', None) is None:
            sigma = SIGMA
        ...
        for k in range(B):
            ...
            if self.training:
                # indicator is not order, there may be repeated element in top-k, because it is soft top-k.
                indicator = PerturbedTopKFunction.apply(
                    scores,
                    top_k, self.num_samples, sigma)  # b k d : 1, top_k, N, e.g., torch.Size([1, 32, 736]) torch.float32
                # indicator = einops.rearrange(indicator, "b k d -> b d k")  #
                # indicators = rearrange(indicators, "b d k -> b k d")
                patches = torch.einsum("b k d, b d c -> b k c", indicator, x_k)  # b, k, c, (1, top_k, c)

                # TODO: How to get the indices of the top k locations? Does PerturbedTopKFunction has the same
                #  top k location with HardTopK?
            ...
            # using indices to modify is dangerous, we need to keep the indices and tokens_small_obj_new
            # to be in consistent order.
            # Here indices is only one-dimensional, indicator use indices from small to large, so x_k is also
            # from small index token to large index token; indices sorted, so they are consistented.
            # tokens_small_obj_new and x_new has the same order as indices
