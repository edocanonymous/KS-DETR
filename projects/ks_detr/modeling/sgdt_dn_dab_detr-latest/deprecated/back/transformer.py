...
from models.sgdt.sgdt_components import resize_sgdt_target
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
        ...
        if encoder_layer_config is not None:
            assert isinstance(encoder_layer_config, str)
                ...
                for l_conf in layer_conf_split:
                    ...
                    if l_type == 'regular':
                        ...
                        encoder_layer = TransformerEncoderSGDTLayer(
                            ...
                            token_scoring_version=token_scoring_version,
                        )
                    elif l_type == 'sgdtv1':
        ...
        # self.encoder_sgdt_layer_version = encoder_sgdt_layer_version
        # self.num_encoder_sgdt_layers = num_encoder_sgdt_layers
        # if num_encoder_layers + num_encoder_sgdt_layers == 0:
        #     # To conduct experiments without encoder.
        #     self.encoder = TransformerEmptyEncoder()
        # else:
        #     encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
        #                                             dropout, activation, normalize_before,
        #                                             )
        #
        #     encoder_sgdt_layer = None
        #     if self.num_encoder_sgdt_layers > 0:
        #         if self.encoder_sgdt_layer_version == 0:
        #             encoder_sgdt_layer = TransformerEncoderSGDTLayer(
        #                 d_model, nhead, dim_feedforward,
        #                 dropout, activation, normalize_before,
        #                 # reclaim_padded_region=reclaim_padded_region,
        #                 token_scoring_version=token_scoring_version,
        #             )
        #         elif self.encoder_sgdt_layer_version > 0:
        #             encoder_sgdt_layer = TransformerEncoderSGDTLayerV1(
        #                 d_model, nhead, dim_feedforward,
        #                 dropout, activation, normalize_before,
        #                 # ----------------
        #                 token_scoring_version=token_scoring_version,
        #             )
            ...
            #                                   encoder_sgdt_layer=encoder_sgdt_layer,
            #                                   num_encoder_sgdt_layers=num_encoder_sgdt_layers
            #                                   )
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed, tgt, attn_mask=None,
        ...
        # update the pos_embed is necessary.
        if sgdt_output_list is not None and len(sgdt_output_list) > 0:

            # # save the feat_map_size for generating the gt information for score supervision.
            #
            # # map_size=src.shape,  # torch.Size([672, 2, 256])
            feat_map_size = torch.tensor([h, w]).to(memory.device)
            for sgdt_output in sgdt_output_list:
                sgdt_output['feat_map_size'] = feat_map_size.clone()

            sgdt_output = sgdt_output_list[-1]
            if 'adapted_pos' in sgdt_output and sgdt_output['adapted_pos'] \
                    and not self.encoder_without_pos:  #
                # adapted original pos_embed
                pos_embed = extract_adapted_token_pos_embed(sgdt_output, pos=pos_embed)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        sgdt_output_list = []
        if self.with_sgdt_layer:
            for layer_id, layer in enumerate(self.sgdt_layers):
                # ------------------- TTI Modification
                # token adaption here, input: a set of tokens, output: a set of tokens.
                # TODO: for sgdt layer, the pos_scaling should be disabled, because the padded tokens
                ...
                token_scoring_version = layer.token_scoring_version
                token_scoring_version_parser = TokenScoringConfigParser(token_scoring_version)
                if token_scoring_version is not None and \
                        token_scoring_version_parser.reclaim_padded_region_():  #
        ...
        return output, sgdt_output_list
...
class TransformerEncoderSGDTLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        # --------------------------------
        self.sgdt = SGDT_module(embed_dim=d_model,
                                token_scoring_version=token_scoring_version)  # Token adaption module, shared across all layers

        # self.reclaim_padded_region = reclaim_padded_region
        self.token_scoring_version = token_scoring_version
        self.nhead = nhead
        # --------------------------------
    ...
    def forward(self,
        ...
        #     'tokens_to_discard': tokens_to_discard,
        # }
        # fg_score, scale_score  # torch.Size([650, 2, 2]), torch.Size([630, 2, 3]);
        ...
        q_adapted_pos = extract_adapted_token_pos_embed(adapted_token_dict=adapted_token_dict, pos=pos)
        q = self.with_pos_embed(adapted_token_dict['x'], pos=q_adapted_pos)  # using adapted tokens as queries
        ...
        # if self.reclaim_padded_region:
        #     # Todo: to continue here.
        #     # We reuse the padded locations for saving the representations of small and medium objects.
        #     # src_mask
        #     tokens_to_discard = adapted_token_dict['tokens_to_discard']
        #     invalid_tokens = ~(get_valid_token_mask(src, src_key_padding_mask))  # x
        #     # TODO: debug only, to remove in future.
        #     if 'invalid_tokens' in adapted_token_dict:
        #         assert torch.equal(invalid_tokens, adapted_token_dict['invalid_tokens'])
        #
        ...
        #
        #     # # # adapted_token_dict['attn_mask'] saved the information of the reclaimed padded regions
        #     # # # (not necessarily all padded regions are claimed if there is few fg object but larger padded regions).
        #     # # src_mask = adapted_token_dict['attn_mask']  # bool
        #     # if src_mask is not None:
        #     #     raise NotImplementedError('SGDT reclaim_padded_region does not support non-empty attn_mask')
        #     #     # logical and or or?
        #     #     # src_mask = src_mask.logical_or(attn_mask)  # ######
        #
        #     # src2 = self.self_attn(q, k, value=src, attn_mask=attn_mask,
        #     #                       key_padding_mask=None)[0]
        # # else:
        # #     src_mask_reclaimed = src_key_padding_mask
        ...
        sgdt_output = dict(
            ...
            tokens_small_obj=adapted_token_dict['tokens_small_obj'],
            tokens_to_discard=adapted_token_dict['tokens_to_discard'],
            src_mask_reclaimed=adapted_token_dict['src_mask_reclaimed'],
...
class TransformerEncoderSGDTLayerV1(TransformerEncoderSGDTLayer):
    ...
    def forward(self,
        ...
        adapted_token_dict, fg_score_logit, small_scale_score_logit, valid_tokens = \
            self.sgdt(src, mask=src_key_padding_mask,
                      sgdt_targets=sgdt_targets)  # torch.Size([630, 2, 256]),torch.Size([2, 630])

        k_adapted_pos = extract_adapted_token_pos_embed(
            adapted_token_dict=adapted_token_dict, pos=pos)

        # ---------------------
        k = self.with_pos_embed(adapted_token_dict['x'], pos=k_adapted_pos)  # using adapted tokens as queries
        # ---------------------
        ...
        # valid_tokens = get_valid_token_mask(x, mask)
        sgdt_output = dict(
            # do not save adapted_pos, because the pos for q is not modified, k is modified, but the
            # output corresponding to the q not the k.
            adapted_pos=False,
            fg_score_logit=fg_score_logit,  # B, N; torch.Size([2, 630])
            small_scale_score_logit=small_scale_score_logit,
            valid_tokens=valid_tokens,
...
def build_transformer(args):
    return Transformer(
        ...
        token_scoring_version=args.token_scoring_version,
