...
def get_self_attn_q_k_v(src,
    ...
    if not token_masking and not token_masking_loc:  # default situation, no token setting for masking
        q = k = with_pos_embed(src, pos)
        v = src
        return q, k, v
    else:
        src_with_gt = None
        if token_masking_loc and token_masking_loc in ['X', 'Q', 'K', 'V', 'QK', 'KV', ]:
            ...
            if token_masking == 'sMLP':
                ...
                k_adapted_pos = pos  # just use the original pos.
                src_with_gt = with_pos_embed(sgdt_output['x'], pos=k_adapted_pos)

                # return extract_smlp_qkv(src=src, pos=pos, src_key_padding_mask=src_key_padding_mask, sgdt=sgdt, )
            elif token_masking == 'MarkFg1Bg0':
                src_with_gt = mark_encoder_feature_by_fg_gt(src, sgdt)

                # return extract_mark_fg1_bg0_qkv(src=src, pos=pos, sgdt=sgdt,)
            else:
            ...
            if token_masking_loc == 'X':
                ...
                k = with_pos_embed(src_with_gt, pos)
                v = src_with_gt
            else:  # no update (probably due to , 'MHA_Out', 'FFN_Out')
                q = k = with_pos_embed(src, pos)
                v = src
            return q, k, v


def extract_smlp_qkv(src,
                     pos: Optional[Tensor] = None,
                     src_key_padding_mask: Optional[Tensor] = None,
                     sgdt=None, ):
    pass
...
def extract_mark_fg1_bg0_qkv(src, pos: Optional[Tensor] = None, sgdt=None, ):
    token_masking_loc = sgdt.args.token_masking_loc
    assert token_masking_loc, 'sgdt.args.token_masking_loc is not set.'
