...
from projects.h_deformable_detr.modeling import HDeformableDETR
...
class KSHDeformableDETR(HDeformableDETR):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            lambda_one2many=lambda_one2many,
        )
        self.ksgt = ksgt
    ...
    def forward(self, batched_inputs):
        ...
        # TODO: make multiple-sample inference possible as above.
        if self.training:
            batch_size, _, H, W = images.tensor.shape
            img_masks = images.tensor.new_ones(batch_size, H, W)
            for img_id in range(batch_size):
